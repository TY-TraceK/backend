...
@Tag(name = "visitVerification", description = "투표 관련 API")
    ...
    @ApiResponses(
            value = {
                ...
                @io.swagger.v3.oas.annotations.responses.ApiResponse(
                        responseCode = "400",
                        description =
                                "1. 요청 파라미터 유효성 검증 실패 (필수 필드 누락/공백)\n2. 이미 해당 관광지에 유효한 투표를 완료한 경우 (ALREADY_visitVerificationD)",
                        content = @Content(schema = @Schema(implementation = ApiResponse.class))),
                @io.swagger.v3.oas.annotations.responses.ApiResponse(
                        responseCode = "401",
    ...
    @ApiResponses(
            value = {
                ...
                @io.swagger.v3.oas.annotations.responses.ApiResponse(
                        responseCode = "400",
                        description =
                                "당일 생성된 투표가 아니라서 취소할 수 없는 경우 (visitVerification_CANNOT_BE_CANCELLED)",
                        content = @Content(schema = @Schema(implementation = ApiResponse.class))),
                @io.swagger.v3.oas.annotations.responses.ApiResponse(
                        responseCode = "401",
                        description = "인증 토큰이 누락되었거나 유효하지 않은 경우 (UNAUTHORIZED)",
                ...
                @io.swagger.v3.oas.annotations.responses.ApiResponse(
                        responseCode = "403",
                        description =
                                "투표 소유자가 아닌 유저가 취소를 요청한 경우 (UNAUTHORIZED_visitVerification_ACCESS)",
                        content = @Content(schema = @Schema(implementation = ApiResponse.class))),
                @io.swagger.v3.oas.annotations.responses.ApiResponse(
                        responseCode = "404",
                        description = "존재하지 않는 투표 ID로 취소를 요청한 경우 (visitVerification_NOT_FOUND)",
                        content = @Content(schema = @Schema(implementation = ApiResponse.class)))
            })
    @SecurityRequirement(name = "jwtAuth")
