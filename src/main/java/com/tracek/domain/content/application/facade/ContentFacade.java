...
public class ContentFacade {
    ...
    public ContentDetailResult getContentDetails(Long contentId) {
        ...
        List<LocationContentArtist> mappings =
                locationQueryService.getMappingsByContentId(contentId);
        ...
        Map<Long, ContentDetailResult.ArtistResult> artistResultMap =
                artists.stream()
                        .collect(
                                Collectors.toMap(
                                        ArtistResult::getId,
        ...
        Map<Long, List<ContentDetailResult.ArtistResult>> locationArtistGroupMap =
                mappings.stream()
                        .collect(
                        ...
                        .map(
                                location -> {
                                    List<ContentDetailResult.ArtistResult> relatedArtists =
                                            locationArtistGroupMap.getOrDefault(
                                                    location.getLocationId(), List.of());
                                    return ContentDetailResult.LocationResult.of(
                                            location, relatedArtists);
                                })
                        .toList();

        return ContentDetailResult.from(
                ContentDetailResult.ContentInfo.of(content), locationResults);
    }
}
