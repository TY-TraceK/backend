...
class LocationQueryControllerTest {
    ...
    @BeforeEach
    void setUp() {
        controller = new LocationQueryController(locationQueryService);
    }

    @Test
    @DisplayName("관광지 단건 조회 성공 시 성공 응답으로 감싸서 반환한다")
    void getLocation_success() {
        Location location = LocationTestFixture.newLocation(1L, "경복궁", "PALACE", 100L);
        given(locationQueryService.getLocation(1L)).willReturn(LocationResult.from(location));

        ApiResponse<LocationResponse> response = controller.getLocation(1L);
    ...
    void getRelatedContentAndArtists_success() {
        ...
        given(locationQueryService.getRelatedContentAndArtists(1L)).willReturn(result);
